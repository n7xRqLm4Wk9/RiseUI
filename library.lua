...
-- // Fixed Dragging Logic (Differentiates between a Tap and a Drag)
local function MakeDraggable(dragHandle, targetFrame)
    ...
    dragHandle.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            ...
            dragHandle:SetAttribute("WasDragged", false) -- Reset drag status
    ...
    UserInputService.InputChanged:Connect(function(input)
        if dragging and input == dragInput then
            ...
            if delta.Magnitude > 5 then
                dragHandle:SetAttribute("WasDragged", true)
            end
...
function Luxware:Notification(opts)
    ...
    local AccentLine = Create("Frame", {
        Parent = NotifFrame, BackgroundColor3 = Theme.Accent, Size = UDim2.new(0, 3, 1, -16), Position = UDim2.new(0, 8, 0.5, -22), AnchorPoint = Vector2.new(0, 0.5), BackgroundTransparency = 1
    })
    ...
    -- Animate In
    PlayTween(NotifFrame, {BackgroundTransparency = 0})
    ...
    task.delay(Duration, function()
        -- Animate Out
        PlayTween(NotifFrame, {BackgroundTransparency = 1})
...
function Luxware:CreateWindow(options)
    ...
    -- Fix: Only toggle if the button was purely tapped, not dragged
    OpenBtn.InputEnded:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            if not OpenBtn:GetAttribute("WasDragged") then
                ToggleUI(true)
            end
        end
    end)
    ...
    local function BuildElementsAPI(TargetParent, IsNested)
        ...
        -- // FIXED SLIDER: Pushes track down and expands row if description exists
        function API:CreateSlider(opts)
            local Row = CreateElementRow(opts.Name, opts.Description)
            local hasDesc = opts.Description ~= nil and opts.Description ~= ""
            
            -- Adjust container height to fit text properly
            local rowHeight = hasDesc and 60 or 50
            Row.Size = UDim2.new(WidthScale, WidthOffset, 0, rowHeight)
            ...
            local ValText = Create("TextLabel", {
                Parent = Row, BackgroundTransparency = 1, Size = UDim2.new(0, 40, 0, 20), 
                Position = UDim2.new(1, -50, 0, hasDesc and 10 or 6),
            ...
            local Track = Create("TextButton", {
                Parent = Row, BackgroundColor3 = Theme.Sidebar, Size = UDim2.new(1, -28, 0, 6), Position = UDim2.new(0, 14, 0, trackY), Text = "", AutoButtonColor = false
            })
