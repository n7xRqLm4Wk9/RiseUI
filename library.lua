...
local function jsonEncode(t)
	-- Minimal JSON encoder for flat key-value tables
	local parts = {}
...
local function jsonDecode(s)
	-- Minimal JSON decoder for flat key-value tables
	local result = {}
...
-- Global flags table and config reference (populated per-window)
CyphraUI._flags      = {}
...
function CyphraUI:CreateWindow(settings)
	...
	-- Animate loading bar then dismiss
	task.spawn(function()
	...
	make("Frame", {
		...
		BackgroundColor3 = THEME.Accent,
		BorderSizePixel  = 0,
	}, titleBar):Parent = titleBar
	addCorner(titleBar:FindFirstChildOfClass("Frame") or titleBar, 4)

	-- Wait — easier to just make the dot separately
	...
	function windowObj:CreateTab(name)
		...
		-- Helper: new card frame inside the page
		...
		function tabObj:CreateToggle(settings)
			...
			-- Clickable hitbox over whole card
			local btn = make("TextButton", {
				Size             = UDim2.new(1, 0, 1, 0),
				BackgroundTransparency = 1,
				Text             = "",
		...
		function tabObj:CreateDropdown(settings)
			...
			-- Wrapper that grows to hold the list
			...
			-- Options list (initially hidden, parented to wrapper)
		...
		function tabObj:CreateColorPicker(settings)
			...
			local PRESETS = {
				Color3.fromRGB(233, 69,  96),  -- accent red
				...
				Color3.fromRGB(255, 87,  34),  -- orange
				Color3.fromRGB(0,   188, 212), -- cyan
				Color3.fromRGB(238, 238, 238), -- white
			}

			-- Card auto-sizing: header row (40) + presets row (44) + rgb input (44)
			local card = make("Frame", {
