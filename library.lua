...
local Luxware = {
    Flags = {}, 
    ConfigSettings = {} -- Added to store Config settings from CreateWindow
}
...
-- // Fixed Config System (Now links perfectly with CreateWindow options)
function Luxware:SaveConfiguration()
    if not hasFilesystem then return end
    if not Luxware.ConfigSettings or not Luxware.ConfigSettings.Enabled then return end

    local folderName = Luxware.ConfigSettings.FolderName or "LuxwareConfigs"
    local fileName = Luxware.ConfigSettings.FileName or "MainConfig"
...
function Luxware:LoadConfiguration()
    if not hasFilesystem then return end
    if not Luxware.ConfigSettings or not Luxware.ConfigSettings.Enabled then return end
...
function Luxware:Destroy()
    if CoreGui:FindFirstChild("LuxwarePremium") then CoreGui.LuxwarePremium:Destroy() end
    if LocalPlayer:WaitForChild("PlayerGui"):FindFirstChild("LuxwarePremium") then LocalPlayer.PlayerGui.LuxwarePremium:Destroy() end
end
...
function Luxware:CreateWindow(options)
    ...
    -- Store Config settings globally for the Save/Load functions
    Luxware.ConfigSettings = options.ConfigurationSaving or {Enabled = false}
    ...
    local function BuildElementsAPI(TargetParent, IsNested)
        ...
        -- // Interactive HSV Color Picker (FIXED INVERTED MATH)
        function API:CreateColorPicker(opts)
            ...
            local function UpdateHue(input)
                -- INVERTED MATH FIX: Reverses the math calculation to match visual tracking correctly
                local percent = 1 - math.clamp((input.Position.X - HueSlider.AbsolutePosition.X) / HueSlider.AbsoluteSize.X, 0, 1)
                HueCursor.Position = UDim2.new(1 - percent, 0, 0.5, 0)
                h = percent 
                UpdateColor()
            end
            ...
            function opts:Set(color)
                opts.Color = color
                h, s, v = color:ToHSV()
                -- Reflects the fix so code-loading tracks to the inverted map correctly
